...
class Trainer(object):
    def __init__(self, args):
        ...
        if args.model   == 'DNANet':
            from model.model_DNANet import DNANet
            model       = DNANet(num_classes=1,input_channels=args.in_channels, block=ShuffleV2Block, num_blocks=num_blocks, nb_filter=nb_filter)
    ...
    def training(self,epoch):   # 数据 模型 损失   
        ...
        for i, (data, labels) in enumerate(tbar):  # dataset getitem的返回形式
            ...
            start = time.time()
            preds= self.model(data) # 前向传播 得到包括中间层的多个结果
            loss = 0    # 初始化loss
            for pred in preds:  # 对中间层和最终结果 pred:torch.Size([16, 1, 256, 256])
                loss += self.lossfunc(pred, labels)   # 分别计算损失 pred.max() 8.48 计算损失时会先sigmoid
            loss /= len(preds)  # 最终损失对每一层特征取均值
