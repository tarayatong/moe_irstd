...
class DNANet(nn.Module):
    def __init__(self, num_classes, input_channels, block, num_blocks, nb_filter, stage=4, block_count=4):   # [16, 32, 64, 128, 256] [2,2,2,2]
        ...
        self.down_list = nn.ModuleList([
            nn.Sequential(
                nn.Conv2d(nb_filter[i], nb_filter[0], kernel_size=2**(self.decode_s-i)+1, stride=2**(self.decode_s-1-i), padding=(2**(self.decode_s-i)+1)//2),
                nn.BatchNorm2d(nb_filter[0]),
                # nn.ReLU(),
            ) for i in range(self.decode_s)])
        for i in range(self.decode_s, stage):
            self.down_list.append(nn.Sequential(
                nn.Upsample(scale_factor=2**(i-self.decode_s+1), mode='bilinear', align_corners=True),
                nn.Conv2d(nb_filter[i], nb_filter[0], kernel_size=1, stride=1, padding=0),
                nn.BatchNorm2d(nb_filter[0]),
            ))
        ...
        for j in range (self.block):
            for i in range(self.stage):
                ...
                # else:
                #     self.node_list[i][j] = MambaOut(
                #         in_chans=inp_c,
                #         out_chans=nb_filter[i],
                #         depths=[3],
                #         dims=[96],
                #         )
                    # self.node_list[i][j] = ImageMoEMamba(
                    #     in_channels=inp_c,
                    ...
                    #     patch_size=4,
                    # )
                    # self.qformer = QFormerFast(
                    #         img_size=self.input_size//(2**i),  # 根据您的需求设置
                    #         patch_size=4,
                    #         in_chans=inp_c,
                    #         out_chans=nb_filter[i],
                    #         num_classes=0,
